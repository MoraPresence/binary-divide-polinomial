...
void dividePolynomials(positionOfNext dividend, positionOfNext divisor, positionOfNext result) {
    ...
    while (numberOfUnused != 0 && numberOfBitesOfDivident >= numberOfBitesOfDivisor) {
        ...
        help = 0;
        while (help < numberOfBitesOfDivisor && numberOfUnused != 0 &&
               numberOfBitesOfDivident >= numberOfBitesOfDivisor) {
            if (dividend->used == 0) {
                dividend->used = 1;
                numberOfUsed++;
                numberOfUnused--;
...
void removeFirstZerosFromDividend(positionOfNext dividend,
        positionOfNext result,
        int *numberOfUsed,
        int *numberOfUnused,
        int *numberOfBitesOfDivident) {
    positionOfNext temp;
    temp->next = NULL;
